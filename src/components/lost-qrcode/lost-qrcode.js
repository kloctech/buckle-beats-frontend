...
const LostQRCode = () => {
  ...
  // WhatsApp Link Helper with dynamic QR Code Name integration
  ...
  useEffect(() => {
    ...
    const scanQRCode = async () => {
      try {
        ...
        await axios.post(`${url}/api/qrcode/scan`, scanData);
      } catch (error) {
        console.error('Error recording scan:', error);
      }
    };
  ...
  useEffect(() => {
    const lastSentTime = localStorage.getItem('lastSentTime');
    if (lastSentTime) {
      const cooldownEndTime = parseInt(lastSentTime, 10) + 5 * 60 * 1000; 
      const currentTime = Date.now();
      const timeLeft = cooldownEndTime - currentTime;
        ...
        lostData !== null && (
          <div className="lostqrcode-container">
            <div className="lostqrcode-main-container">
              ...
              {!lostData?.owner?.qrIsLost && (
                <>
                  <p>It seems this {qrCodeName} hasn't been reported lost yet. The owner might not be aware it's missing.</p>
                  {canSendMessage && <p>Please use the message box below to alert them.</p>}
                </>
              )}
              {lostData?.owner?.qrIsLost && (
                    ...
                    <form onSubmit={handleSubmit(onSubmitForm)}>
                      <div className="textarea-group">
                        <textarea
                          ...
                          style={{ padding: "1rem" }}
                          name="message"
                          placeholder={`Leave a note here, your contact details, or mention if you've left the ${qrCodeName} at Lost and Found reception...`}
                          {...register("message", { required: "Please enter the message" })}
                        />
                        {errors.message && <span style={{ color: 'rgb(250, 111, 104)' }}>{errors.message.message}</span>}
                      </div>
                      <div style={{ display: "flex", justifyContent: "center", marginTop: "1rem" }}>
                        <button className="login-button" style={{ width: '60%' }}>Send Message</button>
                      </div>
                    </form>
                  )}
                </>
                ...
                {lostData?.owner?.email && (
                  <li>
                    <a
                      className="contact-link"
                      style={{ textDecoration: "none" }}
                      href={isMobile ? `mailto:${lostData.owner.email}` : `https://mail.google.com/mail/u/0/?view=cm&fs=1&tf=1&source=mailto&to=${lostData.owner.email}`}
                      target="_blank"
                      rel="nofollow noopener noreferrer"
                    >
                      <img src={EmailIcon} alt="Email Icon" style={!isMobile ? { filter: "brightness(0) saturate(100%) invert(97%) sepia(6%) saturate(7341%) hue-rotate(315deg) brightness(104%) contrast(95%)" } : {}}/>
                      <span>{lostData?.owner?.email}</span>
                    </a>
                  </li>
                )}
                {lostData?.owner?.mobileNumber && lostData?.owner?.mobileNumber.trim() !== "" && (
                  <>
                    {/* Call Option */}
                    <li>
                      <a 
                      ...
                      >
                        ...
                        <span>Call: {lostData.owner.mobileNumber}</span>
                      </a>
                    </li>

                    {/* WhatsApp Option matching styling of other icon lines */}
                    <li>
                      <a 
                        style={{ textDecoration: "none" }}
                        className="contact-link"
                        href={getWhatsAppLink(lostData.owner.mobileNumber)}
                        target="_blank"
                        rel="nofollow noopener noreferrer"
                      >
              ...
              <div className="lostqrcode-content">
                {lostData?.owner?.qrIsLost ? (
                  <div>
                    <p>You're doing more than finding a lost item. Each item at RoamSmartTracker holds a precious story, waiting to be continued with your help.</p>
                    <p>Please consent to also sharing your location, and be a hero in this happy reunion.</p>
                  </div>
                ) : (
                  <div>
                    <p>If possible, proceed to the nearest Lost and Found reception and consent to share the location.</p>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <button className="share-button" onClick={handleGetLocation}>
                        <img src={sharelocation} alt="location" /> Share location
                      </button>
